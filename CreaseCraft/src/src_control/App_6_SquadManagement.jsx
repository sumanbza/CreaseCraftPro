...
export default function App() {
  ...
  // Zone Allocator Form State (Zone 1 + Flexible Custom Zones)
  ...
  const currentClub = useMemo(() => {
    if (isPersonalWorkspace) {
      return { id: 'club-personal', name: 'Personal / Independent Workspace', code: 'IND', squads: [] };
    }
    return clubs.find(c => c.id === activeClubId) || clubs[0] || { id: activeClubId, name: 'Cloud Workspace', code: 'CC', squads: [] };
  }, [clubs, activeClubId, isPersonalWorkspace]);

  const isCoachOrAdmin = useMemo(() => {
    if (isPersonalWorkspace) return false;
    const activeMem = memberships.find(m => m.clubId === activeClubId && (m.userId === currentHuman.uid || m.userId === userProfile?.uid));
    const assignedRoles = activeMem?.roles || [activeRole];
    return assignedRoles.some(r => ['Head Coach', 'Coach', 'Bowling Coach', 'Batting Coach', 'Club Admin'].some(coachRole => r.includes(coachRole)));
  }, [activeRole, activeClubId, memberships, currentHuman, userProfile, isPersonalWorkspace]);
  ...
  const clubRosterList = useMemo(() => {
    ...
    const roster = activeMembers.map(m => {
      ...
      if (m.userId === currentHuman.uid) {
        return { id: currentHuman.uid, name: currentHuman.name, roles: rolesArray, squadIds: squadsArray, roleString: rolesArray.join(', ') };
      }
      const fallbackName = m.userId.replace('user-', '').split('@')[0];
      return { id: m.userId, name: fallbackName.charAt(0).toUpperCase() + fallbackName.slice(1), roles: rolesArray, squadIds: squadsArray, roleString: rolesArray.join(', ') };
    });

    if (!roster.some(r => r.id === currentHuman.uid) && userMemberships.some(m => m.clubId === activeClubId)) {
      roster.push({ id: currentHuman.uid, name: currentHuman.name, roles: ['Player'], squadIds: [currentClub.squads?.[0]?.id || ''], roleString: 'Player' });
    }

    return roster;
  }, [memberships, allUsers, activeClubId, currentHuman, userMemberships, currentClub]);
  ...
  const handleOpenCreateZonePlanModal = (planToEdit = null) => {
    if (planToEdit) {
      ...
      // Load saved custom zones or fallback to default 2 & 3
      ...
      <main className="flex-grow max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6">
                            ...
                            <div key={zone.id || zIdx} className="bg-slate-950 p-3.5 rounded-xl border border-indigo-500/30 space-y-2">
                              ...
                              <div className="bg-slate-900 p-2 rounded border border-slate-800 text-[11px] space-y-1">
                                ...
                                <div className="text-slate-400 pt-1 border-t border-slate-800">
                                  <span>Squads: <strong className="text-emerald-400">{assignedSquadNames.length > 0 ? assignedSquadNames.join(', ') : 'All Club Squads'}</strong></span>
                                </div>
                              </div>
                            </div>
                          );
          ...
          <div className="space-y-6">
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">Design Zone 1 net allocations alongside custom zones with free text and multi-squad assignments.</p>
              </div>
              <button
        ...
        {isCoachOrAdmin && activeTab === 'roster' && (
          <div className="space-y-6">
            ...
            {/* SQUAD CREATION & MANAGEMENT PANEL */}
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-4">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 border-b border-slate-800 pb-3">
                <div>
              ...
              <div className="flex flex-wrap gap-2 pt-1">
                {(currentClub.squads || [
                  { id: 'squad-1', name: '1st XI Senior' },
                  { id: 'squad-2', name: '2nd XI Reserves' }
                ]).map((sq) => (
                      ...
                      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 w-full lg:w-auto">
                        ...
                        <div className="flex flex-col space-y-1 w-full sm:w-auto">
                          ...
                          <div className="flex flex-wrap gap-1.5 max-w-xs">
                            {availableRoles.map((roleName, rIdx) => {
                              ...
                              return (
                                <button
                                  ...
                                  onClick={async () => {
                                    let updatedSquads;
                                    if (isAssigned) {
                                      if (currentSquadIds.length === 1) return;
                    ...
                    {newCustomZones.map((zone, zIdx) => {
                      ...
                      return (
                        <div key={zone.id || zIdx} className="bg-slate-900 p-3.5 rounded-xl border border-slate-800 space-y-3">
                          ...
                          {/* MULTI-SQUAD ASSIGNMENT FOR THIS ZONE */}
                          <div className="space-y-1">
