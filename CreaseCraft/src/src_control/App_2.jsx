...
export default function App() {
  ...
  const [activeRole, setActiveRole] = useState('Independent Athlete');
  const [activeSquadId, setActiveSquadId] = useState('squad-1st-xi');
  ...
  const [newNetLanes, setNewNetLanes] = useState([
    { id: 'lane-1', name: 'Lane 1: High Pace Match Sim', batters: 'Steve Smith, Marnus Labuschagne', bowlersText: 'Pat Cummins (36b), Mitchell Starc (48b)' },
    { id: 'lane-2', name: 'Lane 2: Technical Spin & Flight', batters: 'Travis Head, Alex Carey', bowlersText: 'Nathan Lyon (42b), Adam Zampa (36b)' }
  ]);
  const [newCenterWicket, setNewCenterWicket] = useState('T20 Death Overs Match Sim');
  const [newCenterWicketParticipants, setNewCenterWicketParticipants] = useState('1st XI Squad');
  const [newFieldingStation, setNewFieldingStation] = useState('High-Ball & Boundary Relay Catching');
  const [newFieldingParticipants, setNewFieldingParticipants] = useState('Reserve Fast Bowlers');
  ...
  /* STREAMING_CHUNK:Listening to Firebase Auth state... */
  useEffect(() => {
    if (!auth) return;
  ...
  /* STREAMING_CHUNK:Attaching authenticated Firestore collection listeners... */
  useEffect(() => {
    if (!db) {
      setLoading(false);
      return;
    ...
    // Attach listeners once auth is available
    ...
    const unSubPersonalPlans = onSnapshot(collection(db, 'personal_plans'), (snapshot) => {
      ...
      console.error("Personal plans listener error:", err);
      setLoading(false);
    });
  ...
  const handleSignOut = async () => {
    ...
    setActiveRole('Independent Athlete');
    setAuthMode('SIGNIN');
    setShowAuthModal(true);
    triggerNotify('Signed out of CreaseCraft.', 'info');
  };

  /* STREAMING_CHUNK:Filtering roster and memberships for active user... */
  const clubRosterList = useMemo(() => {
    const defaultRoster = [
      { id: 'user-starc', name: 'Mitchell Starc', role: 'Fast Bowler' },
      { id: 'user-cummins', name: 'Pat Cummins', role: 'Fast Bowler' },
      { id: 'user-smith', name: 'Steve Smith', role: 'Batter' },
      { id: 'user-labuschagne', name: 'Marnus Labuschagne', role: 'Batter' },
      { id: 'user-head', name: 'Travis Head', role: 'All-Rounder' },
      { id: 'user-carey', name: 'Alex Carey', role: 'Wicket-Keeper' },
      { id: 'user-lyon', name: 'Nathan Lyon', role: 'Spin Bowler' },
      { id: 'user-zampa', name: 'Adam Zampa', role: 'Spin Bowler' }
    ];

    const activeMembers = memberships.filter(m => m.clubId === activeClubId);
    const activeUserNames = activeMembers.map(m => {
      if (m.userId === currentHuman.uid) return { id: m.userId, name: currentHuman.name, role: currentHuman.discipline || 'Player' };
      const found = defaultRoster.find(d => d.id === m.userId);
      if (found) return found;
      const formattedName = m.userId.replace('user-', '').replace('gmailcom', '').split('@')[0].split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
      return { id: m.userId, name: formattedName, role: m.roles?.[0] || 'Player' };
    });

    const combined = [...activeUserNames];
    defaultRoster.forEach(d => {
      if (!combined.some(c => c.name.toLowerCase() === d.name.toLowerCase())) {
        combined.push(d);
      }
    });
    return combined;
  }, [memberships, activeClubId, currentHuman]);

  /* STREAMING_CHUNK:Handling club creation and membership joining... */
  const handleCreateClub = async (e) => {
    ...
    try {
      const clubDoc = await addDoc(collection(db, 'clubs'), {
        ...
        code: codeUpper,
        customRoles: ['Bowling Specialist', 'S&C Coach'],
        squads: [
          { id: 'squad-1-' + Date.now(), name: '1st XI Senior' },
      ...
      await addDoc(collection(db, 'memberships'), {
        userId: activeUid,
        clubId: clubDoc.id,
        roles: ['Club Admin', 'Head Coach'],
        squadIds: ['squad-1-' + Date.now()],
        createdAt: new Date().toISOString()
      });

      setActiveClubId(clubDoc.id);
      setActiveRole('Head Coach');
      setActiveTab('matrix');
      setShowClubOnboardingModal(false);
      setNewClubName('');
      setNewClubCode('');
      triggerNotify('Club created and synced to Cloud Firestore!', 'success');
    } catch (err) {
  ...
  const handleJoinClub = async (e) => {
    ...
    if (existing) {
      ...
      setActiveRole(existing.roles?.[0] || 'Player');
      setShowClubOnboardingModal(false);
    ...
    try {
      await addDoc(collection(db, 'memberships'), {
        userId: activeUid,
        clubId: foundClub.id,
        roles: [joinRole || 'Player'],
        squadIds: [foundClub.squads?.[0]?.id || ''],
        createdAt: new Date().toISOString()
      });

      setActiveClubId(foundClub.id);
      setActiveRole(joinRole || 'Player');
      const isCoach = ['Head Coach', 'Coach', 'Bowling Coach', 'Batting Coach', 'Club Admin'].some(r => joinRole.includes(r));
      setActiveTab(isCoach ? 'matrix' : 'dashboard');
      setShowClubOnboardingModal(false);
      setJoinClubCode('');
      triggerNotify('Successfully joined ' + foundClub.name + ' in Cloud Firestore!', 'success');
    } catch (err) {
  ...
  const handleOpenCreateZonePlanModal = (planToEdit = null) => {
    if (planToEdit) {
      ...
      setNewPlanDate(planToEdit.date || '2026-08-04');
      ...
      setNewNetLanes(
        planToEdit.timeSlots?.[0]?.netLanes && planToEdit.timeSlots[0].netLanes.length > 0
          ...
          : [
              { id: 'lane-1', name: 'Lane 1: High Pace Match Sim', batters: 'Steve Smith, Marnus Labuschagne', bowlersText: 'Pat Cummins (36b), Mitchell Starc (48b)' },
              { id: 'lane-2', name: 'Lane 2: Spin & Flight Control', batters: 'Travis Head, Alex Carey', bowlersText: 'Nathan Lyon (42b), Adam Zampa (36b)' }
            ]
      );
      setNewCenterWicket(planToEdit.timeSlots?.[0]?.centerWicket?.title || 'T20 Death Overs Match Sim');
      setNewCenterWicketParticipants(planToEdit.timeSlots?.[0]?.centerWicket?.participants || '1st XI Squad');
      setNewFieldingStation(planToEdit.timeSlots?.[0]?.fieldingStation?.title || 'High-Ball & Boundary Relay Catching');
      setNewFieldingParticipants(planToEdit.timeSlots?.[0]?.fieldingStation?.participants || 'Reserve Fast Bowlers');
    } else {
      setEditingPlanId(null);
      setNewPlanTitle('Tuesday Night Main Squad Microcycle');
      setNewPlanDate('2026-08-04');
      setNewPlanTime('6:00 PM - 7:30 PM');
      setNewNetLanes([
        { id: 'lane-1', name: 'Lane 1: High Pace Match Sim', batters: 'Steve Smith, Marnus Labuschagne', bowlersText: 'Pat Cummins (36b), Mitchell Starc (48b)' },
        { id: 'lane-2', name: 'Lane 2: Spin & Flight Control', batters: 'Travis Head, Alex Carey', bowlersText: 'Nathan Lyon (42b), Adam Zampa (36b)' }
      ]);
      setNewCenterWicket('T20 Death Overs Match Sim');
      setNewCenterWicketParticipants('1st XI Squad');
      setNewFieldingStation('High-Ball & Boundary Relay Catching');
      setNewFieldingParticipants('Reserve Fast Bowlers');
    }
  ...
  const handleSaveZonePlan = async (e) => {
    ...
    const updatedTimeSlots = [
      {
        ...
        centerWicket: { title: newCenterWicket || 'T20 Match Scenario', participants: newCenterWicketParticipants || 'Squad' },
        fieldingStation: { title: newFieldingStation || 'Catching Drills', participants: newFieldingParticipants || 'Reserves' }
      }
    ];
    ...
    try {
      if (editingPlanId) {
        ...
        triggerNotify('Updated multi-zone training plan in Cloud Firestore! ✓', 'success');
      } else {
        await addDoc(collection(db, 'zone_plans'), {
          clubId: activeClubId,
          squadId: activeSquadId || 'squad-1st-xi',
          date: newPlanDate,
          title: newPlanTitle || 'Training Night Microcycle',
          timeSlots: updatedTimeSlots,
          createdAt: new Date().toISOString()
        });
        triggerNotify('Published multi-zone plan to Cloud Firestore! ✓', 'success');
      }
    } catch (err) {
  ...
  /* STREAMING_CHUNK:Saving personal plans to Firestore... */
  ...
  const currentClub = useMemo(() => {
    if (isPersonalWorkspace) {
      return { id: 'club-personal', name: 'Personal / Independent Workspace', code: 'IND' };
    }
    return clubs.find(c => c.id === activeClubId) || clubs[0] || { id: activeClubId, name: 'Cloud Workspace', code: 'CC' };
  }, [clubs, activeClubId, isPersonalWorkspace]);

  const isCoachOrAdmin = useMemo(() => {
    if (isPersonalWorkspace) return false;
    return ['Head Coach', 'Coach', 'Bowling Coach', 'Batting Coach', 'Club Admin'].some(r => activeRole.includes(r));
  }, [activeRole, isPersonalWorkspace]);

  /* STREAMING_CHUNK:Filtering user memberships and personal plans... */
  ...
  const userPersonalPlans = useMemo(() => {
    ...
    return personalPlans.filter(p => 
      ...
      !p.userId // fallback for unassigned items
    );
  ...
  const computeUserACWR = (userId) => {
    ...
    const processed = userLogs.map(s => {
      ...
      const diffDays = Math.max(0, Math.ceil((today - sDate) / (1000 * 60 * 60 * 24)));
  ...
  /* STREAMING_CHUNK:Rendering sign in overlay if unauthenticated... */
  if (!userProfile) {
    return (
  ...
  /* STREAMING_CHUNK:Rendering main application header and navigation... */
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans antialiased overflow-x-hidden">
      ...
      <header className="border-b border-slate-800 bg-slate-900/90 backdrop-blur-md sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex flex-col sm:flex-row justify-between items-center gap-3">
          ...
          {/* Workspace Selector & User Identity Badge */}
          <div className="flex items-center gap-3">
                    ...
                    {userMemberships.length === 0 ? (
                      ...
                      <div className="space-y-1.5 max-h-48 overflow-y-auto pr-1">
                        {userMemberships.map(m => {
                          ...
                          const isCoachRole = ['Head Coach', 'Coach', 'Bowling Coach', 'Batting Coach', 'Club Admin'].some(r => primaryRole.includes(r));
                          ...
                          return (
                            <button
                              ...
                              onClick={() => {
                                ...
                                setActiveSquadId(m.squadIds?.[0] || club.squads?.[0]?.id || '');
                                setActiveTab(isCoachRole ? 'matrix' : 'dashboard');
                                setShowWorkspaceDropdown(false);
                              }}
                              className={`w-full text-left p-2.5 rounded-xl border text-xs transition-all flex items-center justify-between ${
      ...
      {/* STREAMING_CHUNK:Rendering main view tab panels... */}
      <main className="flex-grow max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6">
        
        {/* WORKSPACE TAB HEADER NAVIGATION */}
        <div className="flex border-b border-slate-800 mb-6 overflow-x-auto">
          {isCoachOrAdmin ? (
            <>
              ...
              >
                <IconUsers /> Club Roster & Custom Roles
              </button>
            </>
          ) : (
            <>
              ...
              <div className="lg:col-span-2 space-y-4">
                ...
                {userPersonalPlans.length === 0 ? (
                  ...
                  userPersonalPlans.map(plan => {
                    const isPastPlan = plan.date < '2026-08-04';
                      ...
                      <div key={plan.id} className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-4">
                        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b border-slate-800 pb-3">
                          ...
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            {isPastPlan && (
        <button
          onClick={() => !isVerified && handleOpenPersonalPlanModal(plan)} // or your existing modal trigger
          disabled={isVerified}
          className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all ${
            isVerified 
          ...
          <div className="space-y-6">
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 flex justify-between items-center">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">Filter of training plans created by {currentClub.name} coaches.</p>
              </div>
            </div>
            ...
            <div className="space-y-4">
              {zonePlans.filter(p => p.clubId === activeClubId).length === 0 ? (
                ...
                zonePlans.filter(p => p.clubId === activeClubId).map(plan => {
                  ...
                  const isPastOrToday = plan.date <= '2026-08-04';
                  return (
                    <div key={plan.id} className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-4">
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 text-xs">
                        ...
                        <div className="bg-slate-950 p-3.5 rounded-xl border border-amber-500/30 space-y-2">
                          <span className="font-extrabold text-xs text-amber-400 block border-b border-slate-800 pb-1">
                            🏟️ Zone 2: Center Wicket Scenario
                          </span>
                          <div className="bg-slate-900 p-2 rounded border border-slate-800 text-[11px] space-y-1">
                            <span className="font-bold text-slate-200 block text-amber-300">
                              {slot?.centerWicket?.title || 'Match Scenario'}
                            </span>
                            <span className="text-slate-400 block">
                              Group: <strong className="text-slate-200">{slot?.centerWicket?.participants || 'Main Squad'}</strong>
                            </span>
                          </div>
                        </div>
                        ...
                        <div className="bg-slate-950 p-3.5 rounded-xl border border-cyan-500/30 space-y-2">
                          <span className="font-extrabold text-xs text-cyan-400 block border-b border-slate-800 pb-1">
                            🏃 Zone 3: Fielding & Conditioning
                          </span>
                          <div className="bg-slate-900 p-2 rounded border border-slate-800 text-[11px] space-y-1">
                            <span className="font-bold text-slate-200 block text-cyan-300">
                              {slot?.fieldingStation?.title || 'Catching Drills'}
                            </span>
                            <span className="text-slate-400 block">
                              Squad: <strong className="text-slate-200">{slot?.fieldingStation?.participants || 'Reserves'}</strong>
                            </span>
                          </div>
                        </div>
                      </div>
                      ...
                      {isPastOrToday ? (
                        <button
                          ...
                          className="w-full bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-extrabold text-xs py-2.5 rounded-xl transition-all shadow"
                        >
                          Complete & Verify Actual Deliveries ✓
                        </button>
                      ) : (
        ...
        {/* TAB 4: SQUAD HEALTH MATRIX (COACH VIEW) */}
        {isCoachOrAdmin && activeTab === 'matrix' && (
          <div className="space-y-6">
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 flex justify-between items-center">
              <div>
                <h2 className="text-base font-bold text-slate-100 flex items-center gap-2">
                ...
                return (
                  <div key={m.id} className="bg-slate-900 border border-slate-800 rounded-2xl p-4 space-y-3">
                    <div className="flex justify-between items-center">
                      <span className="font-bold text-sm text-white">{m.userId}</span>
        ...
        {/* TAB 5: MULTI-ZONE NET ALLOCATOR (COACH VIEW) */}
        {isCoachOrAdmin && activeTab === 'allocator' && (
          <div className="space-y-6">
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">Design multi-lane net allocations (Zone 1), center wicket scenarios (Zone 2), and fielding routines (Zone 3).</p>
              </div>
              <button
                ...
                return (
                  <div key={plan.id} className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-4">
                    <div className="flex justify-between items-center border-b border-slate-800 pb-3">
                      <div>
                        <h3 className="font-bold text-base text-white">{plan.title}</h3>
                        <span className="text-xs text-emerald-400 font-bold">📅 Date: {plan.date} | ⏰ {slot?.time || '6:00 PM - 7:30 PM'}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => {
        ...
        {/* TAB 6: CLUB ROSTER & CUSTOM ROLES (COACH VIEW) */}
        {isCoachOrAdmin && activeTab === 'roster' && (
          <div className="space-y-6">
            ...
            <div className="bg-gradient-to-r from-emerald-950/60 via-slate-900 to-slate-900 border border-emerald-500/30 rounded-2xl p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div>
                ...
                <h2 className="text-base font-bold text-white mt-1.5">{currentClub.name} - Member Roster & Staff Management</h2>
                <p className="text-xs text-slate-400 mt-0.5">Share the club code with players to allow them to join this workspace.</p>
              </div>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-4">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 border-b border-slate-800 pb-3">
                <div>
                  <h3 className="font-bold text-sm text-slate-100 flex items-center gap-2">
                    🏷️ Club Custom Staff Roles
                  </h3>
                  <p className="text-xs text-slate-400 mt-0.5">Define specialized titles for coaches, physios, and support staff in {currentClub.name}.</p>
                </div>
              </div>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  placeholder="e.g. Throwdown Specialist, Spin Bowling Lead, Physio"
                ...
                <button
                  onClick={async () => {
                    ...
                    await updateDoc(clubRef, {
                      customRoles: [...(currentClub.customRoles || []), newRole]
                    });
                    input.value = '';
                    triggerNotify('Added custom role "' + newRole + '" to Firestore!', 'success');
                  }}
              ...
              <div className="flex flex-wrap gap-2 pt-1">
                {(currentClub.customRoles || ['Bowling Specialist', 'S&C Coach']).map((role, idx) => (
                  <span key={idx} className="bg-slate-950 text-slate-300 border border-slate-800 text-xs px-3 py-1 rounded-xl flex items-center gap-2">
                    <span className="text-emerald-400">•</span> {role}
                    <button
                      onClick={async () => {
                        const clubRef = doc(db, 'clubs', activeClubId);
                        await updateDoc(clubRef, {
                          customRoles: (currentClub.customRoles || []).filter(r => r !== role)
                        });
                        triggerNotify('Removed custom role "' + role + '" in Firestore.', 'info');
                      }}
            ...
            {/* 3. ACTIVE MEMBER ROSTER */}
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-4">
              <div className="border-b border-slate-800 pb-3">
                <h3 className="font-bold text-sm text-slate-100 flex items-center gap-2">
                  👥 Active Club Roster ({memberships.filter(m => m.clubId === activeClubId).length} Members)
                </h3>
                <p className="text-xs text-slate-400 mt-0.5">Assign custom staff titles and squad affiliations to registered club members.</p>
              </div>

              <div className="space-y-3">
                {memberships.filter(m => m.clubId === activeClubId).map(m => {
                  const foundUser = clubRosterList.find(u => u.id === m.userId);
                  const displayName = foundUser ? foundUser.name : m.userId.replace('user-', '').replace('gmailcom', '').split('@')[0].split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
                  const displayRole = foundUser ? foundUser.role : 'Player';
                  ...
                  const availableRoles = [
                    ...
                    'Batting Coach',
                    'Club Admin',
                    ...(currentClub.customRoles || [])
                  ];

                  return (
                    <div key={m.id} className="bg-slate-950 p-4 rounded-xl border border-slate-800 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 text-xs">
                      <div>
                        <div className="flex items-center gap-2">
                      ...
                      <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
                        <div className="flex flex-col">
                          <span className="text-[9px] text-slate-400 uppercase font-bold mb-1">Assigned Role</span>
                          <select
                            value={m.roles?.[0] || 'Player'}
                            onChange={async (e) => {
                              const newR = e.target.value;
                              const memRef = doc(db, 'memberships', m.id);
                              await updateDoc(memRef, { roles: [newR] });
                              triggerNotify('Updated ' + displayName + '\'s role to "' + newR + '" in Firestore!', 'success');
                            }}
                            className="bg-slate-900 border border-slate-800 text-slate-200 rounded-xl px-3 py-1.5 text-xs focus:outline-none focus:border-emerald-500"
                          >
      ...
      {/* STREAMING_CHUNK:Rendering create plan and club onboarding modals... */}
      {showCreatePlanModal && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-2xl p-6 shadow-2xl space-y-4 my-8">
            ...
            <form onSubmit={isCoachOrAdmin && activeTab === 'allocator' ? handleSaveZonePlan : handleSavePersonalPlan} className="space-y-4">
              <div>
                ...
                <div className="space-y-4 pt-2">
                  <div className="bg-slate-950 p-4 rounded-xl border border-emerald-500/30 space-y-3">
                    ...
                    {newNetLanes.map((lane, index) => (
                      <div key={lane.id || index} className="bg-slate-900 p-3 rounded-xl border border-slate-800 space-y-2">
                        <input
                          type="text"
                          value={lane.name}
                          onChange={(e) => {
                        ...
                        <input
                          ...
                          placeholder="Batters list"
                          className="bg-slate-950 border border-slate-800 text-slate-200 text-xs p-2 rounded-lg w-full"
                        />
                        <input
                          type="text"
                          ...
                          placeholder="Bowlers & quotas (e.g. Pat Cummins (36b))"
                          className="bg-slate-950 border border-slate-800 text-slate-200 text-xs p-2 rounded-lg w-full"
                        />
                      </div>
                    ))}
                  ...
                  <div className="bg-slate-950 p-4 rounded-xl border border-amber-500/30 space-y-2">
                    <h4 className="font-extrabold text-xs text-amber-400">🏟️ Zone 2: Center Wicket Scenario</h4>
                    <input
                      type="text"
                      value={newCenterWicket}
                      onChange={(e) => setNewCenterWicket(e.target.value)}
                  ...
                  <div className="bg-slate-950 p-4 rounded-xl border border-cyan-500/30 space-y-2">
                    <h4 className="font-extrabold text-xs text-cyan-400">🏃 Zone 3: Fielding & Conditioning</h4>
                    <input
                      type="text"
                      value={newFieldingStation}
                      onChange={(e) => setNewFieldingStation(e.target.value)}
                      className="w-full bg-slate-900 border border-slate-800 rounded-xl p-2.5 text-xs text-white"
                    />
                  </div>
                </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-md p-6 shadow-2xl space-y-4">
            ...
            {onboardingTab === 'CREATE' ? (
              ...
              <form onSubmit={handleJoinClub} className="space-y-3 pt-1">
                ...
                <div>
                  <label className="text-[10px] text-slate-400 font-bold uppercase block mb-1">Your Primary Role in Club</label>
                  <select
                    value={joinRole}
                  ...
                  >
                    ...
                    <option value="Assistant Bowling Coach">Assistant Bowling Coach</option>
                    <option value="Batting Coach">Batting Coach</option>
                    <option value="S&C Coach">S&C Coach / Physio</option>
                  </select>
                </div>
              ...
              <div>
               ...
               <button
				  onClick={async () => {
					...
					console.log("1. Verify button clicked! Target:", completionTarget);
					try {
					  // Get today's real date dynamically
					  const today = new Date().toISOString().split('T')[0];
					  
					  const logData = {
						userId: currentHuman.uid,
						clubId: activeClubId,
						planId: completionTarget.id, // Add the hard link we discussed!
						scope: isPersonalWorkspace ? 'PERSONAL' : 'CLUB',
						date: today, // Fixed: No longer hard-coded to 2026-08-04
					  ...
					  console.log("2. Attempting to write log to Firestore:", logData);

					  await addDoc(collection(db, 'logs'), logData);
					  
					  console.log("3. Success! Log written to database.");
