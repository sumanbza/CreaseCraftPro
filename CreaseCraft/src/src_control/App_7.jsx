...
export default function App() {
  ...
  // Dynamic RBAC Permission Evaluator (`can` function)
  const can = useMemo(() => {
  ...
  const handleCreateClub = async (e) => {
    ...
    try {
      ...
      const defaultRoles = [
        ...
        { name: 'Head Coach', capabilities: ['manage_squads', 'manage_roster', 'create_zone_plans', 'edit_zone_plans', 'view_full_schedule', 'view_squad_health_matrix'] }
      ];
  ...
  const handleJoinClub = async (e) => {
    ...
    try {
      // Joining via invite code defaults to Player role and an empty squad array (unassigned)
      await addDoc(collection(db, 'memberships'), {
        userId: activeUid,
        clubId: foundClub.id,
        roles: ['Player'],
      ...
      setShowClubOnboardingModal(false);
      setJoinClubCode('');
      triggerNotify('Successfully joined ' + foundClub.name + ' as Player (Unassigned to squad)!', 'success');
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans antialiased overflow-x-hidden">
      ...
      <main className="flex-grow max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          <div className="space-y-6">
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">Hybrid Spotlight View for {currentClub.name}</p>
              </div>
        ...
        {/* TAB 5: MULTI-ZONE NET ALLOCATOR */}
