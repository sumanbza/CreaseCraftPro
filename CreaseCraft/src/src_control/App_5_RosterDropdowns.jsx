    ...
    const roster = activeMembers.map(m => {
      ...
      if (foundUser) {
        return {
          ...
          name: foundUser.name || foundUser.email?.split('@')[0] || 'Member',
          role: m.roles?.[0] || foundUser.discipline || 'Player'
        };
      }
      if (m.userId === currentHuman.uid) {
        return { id: currentHuman.uid, name: currentHuman.name, role: currentHuman.discipline || 'Player' };
      }
      const fallbackName = m.userId.replace('user-', '').split('@')[0];
      return { id: m.userId, name: fallbackName.charAt(0).toUpperCase() + fallbackName.slice(1), role: m.roles?.[0] || 'Player' };
    });

    if (!roster.some(r => r.id === currentHuman.uid) && userMemberships.some(m => m.clubId === activeClubId)) {
      roster.push({ id: currentHuman.uid, name: currentHuman.name, role: currentHuman.discipline || 'Player' });
    }
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-4">
              ...
              <div className="space-y-3">
                {memberships.filter(m => m.clubId === activeClubId).map(m => {
                  ...
                  const displayName = foundUser ? foundUser.name : m.userId.replace('user-', '').split('@')[0];
